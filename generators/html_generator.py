...
def _page_wrapper(title, current_page, feed_data, body_content, extra_scripts='', now=None):
    ...
    if current_page == 'index.html':
        ...
        article_overlay = ('<div id="article-overlay" class="article-overlay hidden">\n'
                          ...
                          '      <div class="article-reader-title" id="article-reader-title"></div>\n'
                          '      <button class="article-reader-close" onclick="closeArticle()" title="Close (Esc)">&times;</button>\n'
                          '    </div>\n'
                          '    <div class="article-reader-scroll" id="article-reader-scroll">\n'
                          '      <div id="article-reader-content"></div>\n'
                          '    </div>\n'
                          '    <div class="article-reader-footer">\n'
                          '      <a id="article-reader-link" href="#" target="_blank" rel="noopener">'
...
def _render_index(feed_data):
    ...
    if news_cards:
        ...
        for nc in sorted_cards:
            ...
            body += ('  <div class="card news-card" onclick="openArticle(this,\'' + _esc(nc.get('url', '')) + '\')" style="cursor:pointer;">\n'
                     + thumb_html +
    ...
    scripts = ('<script src="../feed-loader.js?v=8"></script>\n'
               ...
               '  };\n'
               '  window.openArticle = function(el, articleUrl) {\n'
               '    var ao = document.getElementById(\'article-overlay\');\n'
               '    var titleEl = document.getElementById(\'article-reader-title\');\n'
               '    var contentEl = document.getElementById(\'article-reader-content\');\n'
               '    var linkEl = document.getElementById(\'article-reader-link\');\n'
               '    titleEl.textContent = \'Loading...\';\n'
               '    contentEl.innerHTML = \'<div class="article-loading"><div class="spinner"></div><div>Fetching article...</div></div>\';\n'
               '    linkEl.href = articleUrl;\n'
               '    ao.classList.remove(\'hidden\');\n'
               '    document.body.style.overflow = \'hidden\';\n'
               '    fetch(\'/article-content?url=\' + encodeURIComponent(articleUrl))\n'
               ...
               '          var html = \'\';\n'
               '          if (data.image) {\n'
               '            html += \'<div class="article-reader-thumb"><img src="\' + data.image + \'" alt="" loading="lazy"></div>\';\n'
               '          }\n'
               '          html += \'<h1 class="article-heading">\' + data.title + \'</h1>\';\n'
               '          html += \'<div class="article-meta"><a href="\' + articleUrl + \'" target="_blank" rel="noopener" style="color:var(--accent);">\' + articleUrl + \'</a></div>\';\n'
               '          html += data.content || \'<p>No content available.</p>\';\n'
               '          contentEl.innerHTML = html;\n'
               '        } else {\n'
               '          contentEl.innerHTML = \'<div class="article-error">Failed to load article: \' + (data.error || \'unknown error\') + \'</div>\';\n'
               '        }\n'
               '      })\n'
               '      .catch(function(err) {\n'
               '        contentEl.innerHTML = \'<div class="article-error">Failed to load article.</div>\';\n'
               '      });\n'
               '  };\n'
